...
<%@ page import="com.bean.groupSend_information" %>
...
<head>
    <meta charset="UTF-8">
    <title>${requestScope.groupName}</title>
        ...
        <div id = "relationshipList">
            ...
            <div id = "v2" class = "v">
                <table id = "relationshipViewG" border = "0" cellpadding="0" cellspacing="0">
                    <%
                        int presentId = (int) request.getAttribute("groupId");
                        ...
                        for(int i = 0; i < listG.size(); i++) {
                            ...
                            request.setAttribute("groupId1", groupId);
                            String groupName = information.getGroupName();
                            request.setAttribute("groupName1", groupName);
                            if(groupId != presentId) {
                    %>
                    <tr>
                        <td width="200px">
                            <a id = "relationshipLinkG" href="/ChatSystem/groupChat/refresh?groupId=${groupId1}&groupName=${groupName1}">
                                &nbsp &nbsp <%=information.getGroupName()%> &nbsp (<%=information.getGroupId()%>)
                            </a>
                        </td>
                        <td>
                            <button id = "deleteG" class = "butt" onclick = "window.location.href='../group/delete?groupId=${groupId1}'" type = "button">
                                ×
                            </button>
                        </td>
                    </tr>
...
<div id = "frame2" class = "fra">
    <div id = "component2" class = "comp" >
        <div id = "chatTitle">
            <div id = "chatTitle1">&nbsp &nbsp &nbsp <%=request.getParameter("groupName")%> &nbsp (<%=request.getParameter("groupId")%>)</div>
            <button id = "chatBack" class = "butt" onclick = "window.location.href='../relationshipList/refresh'" type = "button">
                ×
            </button>
        </div>
        <div id = "chatScreen">
            <table id = "chatView" border = "0" cellpadding="0" cellspacing="0">
                <%
                    ...
                    for(int i = 0; i < list2.size(); i++) {
                        groupSend_information sendChatInformation = (groupSend_information) list2.get(i);
                        String record = sendChatInformation.getGroupRecordContent();
                        String sendName = sendChatInformation.getGroupSendName();
                        String sendTime = sendChatInformation.getGroupSendTime();
                %>
                <%
                    if(Objects.equals(sendChatInformation.getGroupSendId(), (Integer) session.getAttribute("userId"))) {
                %>
                <tr align="right">
                    <td>
        ...
        <div id = "sendDiv">
            <form id = "sendForm" action = "/ChatSystem/groupChat/send" method = "get">
                <textarea name = "content"></textarea>
                <input type = "submit" value = "发送" />
                <input type = "hidden" name = "groupId" value = ${requestScope.groupId} />
                <input type = "hidden" name = "groupName" value = ${requestScope.groupName} />
            </form>
        </div>
        <button id = "openHistory" onclick = "window.location.href='../groupHistory/refresh?groupId=${requestScope.groupId}&groupName=${requestScope.groupName}'" type = "button">
            历史
        </button>
    </div>
</div>
